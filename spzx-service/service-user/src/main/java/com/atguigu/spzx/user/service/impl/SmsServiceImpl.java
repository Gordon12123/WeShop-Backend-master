...
SmsServiceImpl implements SmsService {
    ...
    public void sendValidateCode(String phone) {
        ...
        if(StringUtils.hasText(code)){//如果字符串不是 null，不是空字符串("")，并且至少包含一个非空白字符，则返回 true否则false
            //如果 code 包含实际文本内容，那么 if 语句内的代码会执行，即 return; 语句会被执行，方法会立即返回，不再执行后续的代码。
            return;
        }
        //1. 生成验证码
...
//smsSignId（短信前缀）和templateId（短信模板），可登录国阳云控制台自助申请。参考文档：0.11111
